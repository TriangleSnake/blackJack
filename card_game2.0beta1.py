...
def show_current_card(lst):#在牌桌上顯示遊戲中上一張所出的牌
    ...
    for i in lst:
        img_pil = Image.open("/assets/"+str(i)+'.png')
...
def game_over():#當觸發失敗條件時的後續動作
    ...
    #顯示失敗圖片
    loss_img = Image.open('/assets/game_over.png')
...
def win():#檢查勝利條件是否成立及後續動作
    ...
    #顯示勝利圖片
    win_img = Image.open('/assets/win.png').resize((500, 500))
...
#蓋牌的圖片
img_p_o = Image.open('/assets/fold.png')
